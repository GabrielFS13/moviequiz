...
function App() {
  ...
  function normalizeString(str) {
    return str
      ...
      .replace(/[\u0300-\u036f]/g, "") // Regex pra remover acentos
      .replace(/[^a-z0-9\s]/gi, "") // Remove pontuação
      .trim();
  }

  function checkAnswer(answer, movieTitle, originalTitle) {
    const userInput = normalizeString(answer);
    const titlePT = normalizeString(movieTitle);
    const titleOriginal = normalizeString(originalTitle);

    const regex = new RegExp(userInput, 'i'); // 'i' ignora maiúsculas

    return regex.test(titlePT) || regex.test(titleOriginal);
  }
  ...
  function verifica(e) {
    ...
    if (checkAnswer(answer, movieTitle, originalTitle)) {
      setErr(10)
      setAnimation('win')
